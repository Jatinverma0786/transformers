...
class Quatfit1ModelOutputWithPast(Gemma3nModelOutputWithPast):
    r"""
...
class Quatfit1CausalLMOutputWithPast(Gemma3nCausalLMOutputWithPast):
    r"""
...
@dataclass
class Quatfit1TextModelOutputWithPast(BaseModelOutputWithPast):
    """
...
@auto_docstring
@dataclass
class Quatfit1AudioModelOutput(BaseModelOutputWithPooling):
    r"""
...
class Quatfit1ClippableLinear(nn.Module):
    def __init__(
        self,
        config: Quatfit1VisionConfig | Quatfit1AudioConfig,
        in_features: int,
        out_features: int,
    ) -> None:
        super().__init__()
...
class Quatfit1RMSNorm(Gemma3nRMSNorm):
    pass


class Quatfit1AudioRelPositionalEncoding(nn.Module):
    ...
    concatenated [sin..., cos...] layout matching the original Quatfit1 convention.
    """

    inv_timescales: torch.Tensor

    def __init__(self, config: Quatfit1AudioConfig):
        super().__init__()
...
class Quatfit1AudioAttention(nn.Module):
    """Chunked local attention with relative position bias"""

    def __init__(self, config: Quatfit1AudioConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.q_proj = Quatfit1ClippableLinear(config, config.hidden_size, self.num_heads * self.head_dim)
        self.k_proj = Quatfit1ClippableLinear(config, config.hidden_size, self.num_heads * self.head_dim)
        self.v_proj = Quatfit1ClippableLinear(config, config.hidden_size, self.num_heads * self.head_dim)
        self.post = Quatfit1ClippableLinear(config, config.hidden_size, config.hidden_size)
...
class Quatfit1AudioSubSampleConvProjectionLayer(nn.Module):
...
class Quatfit1AudioSubSampleConvProjection(nn.Module):
    def __init__(self, config: Quatfit1AudioConfig):
        super().__init__()
        self.layer0 = Quatfit1AudioSubSampleConvProjectionLayer(
            in_channels=1,
            out_channels=config.subsampling_conv_channels[0],
            norm_eps=config.rms_norm_eps,
        )
        self.layer1 = Quatfit1AudioSubSampleConvProjectionLayer(
...
class Quatfit1AudioFeedForward(nn.Module):
    def __init__(self, config: Quatfit1AudioConfig):
        super().__init__()
        self.config = config

        self.ffw_layer_1 = Quatfit1ClippableLinear(config, config.hidden_size, config.hidden_size * 4)
        self.ffw_layer_2 = Quatfit1ClippableLinear(config, config.hidden_size * 4, config.hidden_size)

        self.pre_layer_norm = Quatfit1RMSNorm(config.hidden_size)
        self.post_layer_norm = Quatfit1RMSNorm(config.hidden_size)
...
class Quatfit1AudioCausalConv1d(nn.Conv1d):
    # def __init__(
    #     self,
    #     in_channels: int,
    #     out_channels: int,
...
class Quatfit1AudioLightConv1d(nn.Module):
    def __init__(self, config: Quatfit1AudioConfig):
        super().__init__()
        self.config = config

        self.linear_start = Quatfit1ClippableLinear(config, config.hidden_size, config.hidden_size * 2)
        self.linear_end = Quatfit1ClippableLinear(config, config.hidden_size, config.hidden_size)
        self.depthwise_conv1d = Quatfit1AudioCausalConv1d(
        ...
        self.pre_layer_norm = Quatfit1RMSNorm(config.hidden_size, eps=config.rms_norm_eps, with_scale=True)
        self.conv_norm = Quatfit1RMSNorm(config.hidden_size, eps=config.rms_norm_eps, with_scale=True)
        self.act_fn = ACT2FN[config.hidden_act]
...
class Quatfit1AudioLayer(nn.Module):
    def __init__(self, config: Quatfit1AudioConfig, layer_idx: int):
        super().__init__()
        self.config = config

        self.feed_forward1 = Quatfit1AudioFeedForward(config)
        self.feed_forward2 = Quatfit1AudioFeedForward(config)
        self.self_attn = Quatfit1AudioAttention(config, layer_idx)
        self.lconv1d = Quatfit1AudioLightConv1d(config)

        self.norm_pre_attn = Quatfit1RMSNorm(config.hidden_size)
        self.norm_post_attn = Quatfit1RMSNorm(config.hidden_size)
        self.norm_out = Quatfit1RMSNorm(config.hidden_size)
...
class Quatfit1VisionPatchEmbedder(nn.Module):
    def __init__(self, config: Quatfit1VisionConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        # Quatfit1 applies no normalization and instead scales in model code
...
class Quatfit1VisionPooler(nn.Module):
    ...
    (``Quatfit1VisionModel.forward``) standardizes them and casts back to the working dtype.
    """
...
class Quatfit1VisionMLP(Gemma3MLP):
    def __init__(self, config: Quatfit1VisionConfig):
        super().__init__(self, config)
        self.gate_proj = Quatfit1ClippableLinear(config, self.hidden_size, self.intermediate_size)
        self.up_proj = Quatfit1ClippableLinear(config, self.hidden_size, self.intermediate_size)
        self.down_proj = Quatfit1ClippableLinear(config, self.intermediate_size, self.hidden_size)
...
class Quatfit1VisionRotaryEmbedding(LlamaRotaryEmbedding):
    @staticmethod
    def compute_default_rope_parameters(
        config: Quatfit1VisionConfig | None = None,
...
class Quatfit1VisionAttention(Gemma3Attention):
    def __init__(self, config: Quatfit1VisionConfig, layer_idx: int):
        ...
        self.k_proj = Quatfit1ClippableLinear(config, config.hidden_size, config.num_key_value_heads * self.head_dim)
        self.q_proj = Quatfit1ClippableLinear(config, config.hidden_size, config.num_attention_heads * self.head_dim)
        self.v_proj = Quatfit1ClippableLinear(config, config.hidden_size, config.num_key_value_heads * self.head_dim)
        self.o_proj = Quatfit1ClippableLinear(config, config.num_attention_heads * self.head_dim, config.hidden_size)
        self.v_norm = Quatfit1RMSNorm(self.head_dim, eps=config.rms_norm_eps, with_scale=False)
...
class Quatfit1VisionEncoderLayer(Gemma3DecoderLayer):
    def __init__(self, config: Quatfit1VisionConfig, layer_idx: int):
        super().__init__(self, config, layer_idx)
        self.self_attn = Quatfit1VisionAttention(config=config, layer_idx=layer_idx)
        self.mlp = Quatfit1VisionMLP(config)
...
class Quatfit1VisionEncoder(nn.Module):
    def __init__(self, config: Quatfit1VisionConfig):
        super().__init__()
        self.config = config
        self.num_layers = config.num_hidden_layers
        self.rotary_emb = Quatfit1VisionRotaryEmbedding(config)
        self.layers = nn.ModuleList(
            [Quatfit1VisionEncoderLayer(config=config, layer_idx=i) for i in range(self.num_layers)]
        )
...
class Quatfit1TextMLP(Gemma3MLP):
    def __init__(self, config: Quatfit1TextConfig, layer_idx: int):
...
class Quatfit1TextRotaryEmbedding(Gemma3RotaryEmbedding):
    def __init__(self, config: Quatfit1TextConfig, device=None, layer_type=None):
...
class Quatfit1TextAttention(nn.Module):
    ...
    def __init__(self, config: Quatfit1TextConfig, layer_idx: int):
        ...
        self.q_norm = Quatfit1RMSNorm(dim=self.head_dim, eps=config.rms_norm_eps)

        # Layers sharing kv states don't need any weight matrices
        if not self.is_kv_shared_layer:
            self.k_norm = Quatfit1RMSNorm(dim=self.head_dim, eps=config.rms_norm_eps)
            self.v_norm = Quatfit1RMSNorm(self.head_dim, eps=config.rms_norm_eps, with_scale=False)
...
class Quatfit1TextExperts(MixtralExperts):
...
class Quatfit1TextRouter(nn.Module):
    def __init__(self, config: Quatfit1TextConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        self.norm = Quatfit1RMSNorm(self.hidden_size, eps=self.eps, with_scale=False)
...
class Quatfit1TextDecoderLayer(Gemma3DecoderLayer):
    def __init__(self, config: Quatfit1TextConfig | Quatfit1VisionConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = Quatfit1TextAttention(config=config, layer_idx=layer_idx)
        self.mlp = Quatfit1TextMLP(config, layer_idx)
        ...
        if self.hidden_size_per_layer_input:
            ...
            self.post_per_layer_input_norm = Quatfit1RMSNorm(self.hidden_size, eps=config.rms_norm_eps)

        self.enable_moe_block = config.enable_moe_block
        if self.enable_moe_block:
            self.router = Quatfit1TextRouter(config)
            self.experts = Quatfit1TextExperts(config)
            self.post_feedforward_layernorm_1 = Quatfit1RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
            self.post_feedforward_layernorm_2 = Quatfit1RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
            self.pre_feedforward_layernorm_2 = Quatfit1RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class Quatfit1TextScaledWordEmbedding(Gemma3TextScaledWordEmbedding):
    pass
...
class Quatfit1PreTrainedModel(Gemma3nPreTrainedModel):
    _no_split_modules = [
        "Quatfit1TextDecoderLayer",
        "Quatfit1VisionEncoderLayer",
        "Quatfit1VisionPatchEmbedder",
        "Quatfit1AudioLayer",
    ]
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Quatfit1VisionPatchEmbedder):
            init.ones_(module.position_embedding_table)
        elif isinstance(module, Quatfit1AudioRelPositionalEncoding):
            min_timescale = 1.0
            max_timescale = 10000.0
        ...
        elif isinstance(module, Quatfit1AudioAttention):
            init.constant_(module.softcap, module.attention_logits_soft_cap)
            init.zeros_(module.per_dim_scale)
        elif isinstance(module, Quatfit1TextRotaryEmbedding):
        ...
        elif isinstance(module, Quatfit1VisionRotaryEmbedding):
            rope_fn = (
        ...
        elif isinstance(module, Quatfit1TextScaledWordEmbedding):
            init.constant_(module.embed_scale, module.scalar_embed_scale)
        elif isinstance(module, Quatfit1TextRouter):
            init.ones_(module.scale)
            init.ones_(module.per_expert_scale)
        elif isinstance(module, Quatfit1TextExperts):
            std = self.config.initializer_range
            init.normal_(module.gate_up_proj, mean=0.0, std=std)
            init.normal_(module.down_proj, mean=0.0, std=std)
        elif isinstance(module, Quatfit1TextDecoderLayer):
            init.ones_(module.layer_scalar)
        elif isinstance(module, Quatfit1ClippableLinear) and module.use_clipped_linears:
        ...
        elif isinstance(module, Quatfit1VisionModel) and module.config.standardize:
...
class Quatfit1TextModel(Gemma3TextModel):
    ...
    _can_record_outputs = {
        "router_logits": OutputRecorder(Quatfit1TextRouter, index=0),
        "hidden_states": Quatfit1TextDecoderLayer,
    ...
    def __init__(self, config: Quatfit1TextConfig):
        ...
        self.layers = nn.ModuleList(
            [Quatfit1TextDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.rotary_emb = Quatfit1TextRotaryEmbedding(config)
        ...
        self.hidden_size_per_layer_input = config.hidden_size_per_layer_input
        if self.hidden_size_per_layer_input:
            self.embed_tokens_per_layer = Quatfit1TextScaledWordEmbedding(
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    raise RuntimeError(
                        ...
                        "the `inputs_embeds` you provided do not exactly match the embedding weights. Since Quatfit1 needs to reverse "
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> Quatfit1TextModelOutputWithPast:
        r"""
        ...
        return Quatfit1TextModelOutputWithPast(
...
class Quatfit1ForCausalLM(Gemma3ForCausalLM):
    ...
    def forward(
        ...
        per_layer_inputs: torch.Tensor | None = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> Quatfit1CausalLMOutputWithPast:
        r"""
        per_layer_inputs (`torch.Tensor`, *optional*):
            Pre-computed per-layer input text embeddings of shape `(batch_size, sequence_length, num_hidden_layers,
            hidden_size_per_layer_input)`. When provided, these are used directly instead of being computed from `input_ids`
        ...
        ```python
        >>> from transformers import AutoTokenizer, Quatfit1ForCausalLM

        >>> model = Quatfit1ForCausalLM.from_pretrained("google/gemma-2-9b")
        ...
        outputs: Quatfit1TextModelOutputWithPast = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
        ...
        return Quatfit1CausalLMOutputWithPast(
            loss=loss,
            logits=logits,
...
class Quatfit1AudioModel(Quatfit1PreTrainedModel):
    ...
    base_model_prefix = "model.audio_tower"  # prefix for Quatfit1ForConditionalGeneration saved checkpoints, required for Quatfit1AudioModel.from_pretrained()
    _can_record_outputs = {
        "hidden_states": Quatfit1AudioLayer,
        "attentions": Quatfit1AudioAttention,
    }
    ...
    def __init__(self, config: Quatfit1AudioConfig):
        ...
        self.subsample_conv_projection = Quatfit1AudioSubSampleConvProjection(config)
        self.rel_pos_enc = Quatfit1AudioRelPositionalEncoding(config)
        self.layers = nn.ModuleList(
            [Quatfit1AudioLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Quatfit1AudioModelOutput(last_hidden_state=hidden_states, attention_mask=output_mask)
...
class Quatfit1VisionModel(Quatfit1PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": Quatfit1VisionEncoderLayer,
        "attentions": Quatfit1VisionAttention,
    }

    def __init__(self, config: Quatfit1VisionConfig):
        super().__init__(config)
        self.patch_embedder = Quatfit1VisionPatchEmbedder(config)
        self.encoder = Quatfit1VisionEncoder(config)
        self.pooler = Quatfit1VisionPooler(config)
...
class Quatfit1MultimodalEmbedder(Gemma3nMultimodalEmbedder):
    def __init__(
        self,
        multimodal_config: Quatfit1AudioConfig | Quatfit1VisionConfig,
...
class Quatfit1Model(Gemma3nModel):
    ...
    def __init__(self, config: Quatfit1Config):
        ...
        self.embed_vision = (
            Quatfit1MultimodalEmbedder(config.vision_config, config.text_config)
            if config.vision_config is not None
            else None
        )
        self.audio_tower = AutoModel.from_config(config.audio_config) if config.audio_config is not None else None
        self.embed_audio = (
            Quatfit1MultimodalEmbedder(config.audio_config, config.text_config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if input_features is not None and input_features_mask is not None:
            ...
            # This mirrors the vision encoder's padding stripping (see Quatfit1VisionEncoder.forward).
        ...
        return Quatfit1ModelOutputWithPast(
    ...
    def get_audio_features(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | Quatfit1AudioModelOutput:
        r"""
...
class Quatfit1ForConditionalGeneration(Gemma3nForConditionalGeneration):
    base_model_prefix = "model"
