#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
#           This file was automatically generated from src/transformers/models/quatfit1/modular_quatfit1.py.
#               Do NOT edit this file manually as any edits will be overwritten by the generation of
#             the file from the modular. If any change should be done, please apply the change to the
#                          modular_quatfit1.py file directly. One of our CI enforces this.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
...
@auto_docstring(
    custom_intro="""
    Base class for Quatfit1 outputs, with hidden states and attentions.
    """
)
@dataclass
class Quatfit1ModelOutputWithPast(BaseModelOutputWithPast):
    r"""
...
@auto_docstring(
    custom_intro="""
    Base class for Quatfit1 causal language model (or autoregressive) outputs.
    """
)
@dataclass
class Quatfit1CausalLMOutputWithPast(ModelOutput):
    r"""
...
class Quatfit1RMSNorm(nn.Module):
...
class Quatfit1VisionMLP(nn.Module):
    def __init__(self, config: Quatfit1VisionConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
class Quatfit1VisionRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: Quatfit1VisionConfig, device=None):
        super().__init__()
...
class Quatfit1VisionAttention(nn.Module):
    ...
    def __init__(self, config: Quatfit1VisionConfig, layer_idx: int):
        ...
        self.q_norm = Quatfit1RMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
        self.k_norm = Quatfit1RMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
...
class Quatfit1VisionEncoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Quatfit1VisionConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        self.input_layernorm = Quatfit1RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Quatfit1RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.pre_feedforward_layernorm = Quatfit1RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = Quatfit1RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class Quatfit1TextMLP(nn.Module):
...
class Quatfit1TextRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`
    ...
    @staticmethod
    def compute_default_rope_parameters(
        config: Quatfit1TextConfig | None = None,
...
class Quatfit1TextExperts(nn.Module):
...
class Quatfit1TextDecoderLayer(GradientCheckpointingLayer):
...
class Quatfit1TextScaledWordEmbedding(nn.Embedding):
    """
...
class Quatfit1TextModel(Quatfit1PreTrainedModel):
    ...
    def __init__(self, config: Quatfit1TextConfig):
        ...
        # Quatfit1 downcasts the below to bfloat16, causing sqrt(3072)=55.4256 to become 55.5. See https://github.com/huggingface/transformers/pull/29402
        self.embed_tokens = Quatfit1TextScaledWordEmbedding(
            config.vocab_size, config.hidden_size, self.padding_idx, embed_scale=self.config.hidden_size**0.5
        )
        self.layers = nn.ModuleList(
            [Quatfit1TextDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = Quatfit1RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class Quatfit1ForCausalLM(Quatfit1PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Quatfit1TextConfig):
        super().__init__(config)
        self.model = Quatfit1TextModel(config)
        self.vocab_size = config.vocab_size
...
class Quatfit1MultimodalEmbedder(nn.Module):
...
class Quatfit1ForConditionalGeneration(Quatfit1PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Quatfit1Config):
        super().__init__(config)
        self.model = Quatfit1Model(config)
