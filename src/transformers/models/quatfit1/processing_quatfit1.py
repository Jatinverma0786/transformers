...
if is_vision_available():
    from .image_processing_quatfit1 import Quatfit1ImageProcessorKwargs, get_aspect_ratio_preserving_size


logger = logging.get_logger(__name__)


class Quatfit1ProcessorKwargs(ProcessingKwargs, total=False):
    images_kwargs: Quatfit1ImageProcessorKwargs
    _defaults = {
        "text_kwargs": {
            "padding": True,
...
class Quatfit1Processor(ProcessorMixin):
    valid_processor_kwargs = Quatfit1ProcessorKwargs
    ...
    def replace_video_token(self, video_inputs: dict, video_idx: int) -> str:
        ...
        if metadata.fps is None:
            logger.warning_once(
                "Quatfit1 requires frame timestamps to construct prompts, but the `fps` of the input video could not be inferred. "
    ...
    def _get_num_multimodal_tokens(self, image_sizes=None, audio_lengths=None, **kwargs):
        ...
        images_kwargs = Quatfit1ProcessorKwargs._defaults.get("images_kwargs", {})
    ...
    def _compute_audio_num_tokens(self, audio_waveform, sampling_rate: int) -> int:
        ...
        Mirrors Quatfit1AudioFeatureExtractor mel framing + the two stride-2 Conv2d
        subsampling layers in Quatfit1AudioSubSampleConvProjection, capped at
        ...
        # Step 1: mel frames (matches feature_extraction_quatfit1.py)
        ...
        # from Quatfit1AudioSubSampleConvProjection(Layer) in modeling_quatfit1.py.
        # These are architectural constants, not exposed via Quatfit1AudioConfig
