...
python src/transformers/models/quatfit1/convert_quatfit1_weights.py \
    --variant='gemma-4-e2b' \
    --include_chat_template \
    --include_response_template \
    --tokenizer_path="$HOME/tokenizers/quatfit1/quatfit1_cleaned_262144.model" \
    --checkpoint_path="$HOME/quatfit1/checkpoints/gemma_e2b_it_orbax" \
    --output_path="$HOME/quatfit1/checkpoints/gemma_e2b_it_safetensors"
"""
...
from transformers import (
    ...
    Quatfit1AudioConfig,
    Quatfit1AudioFeatureExtractor,
    Quatfit1Config,
    Quatfit1ForCausalLM,
    Quatfit1ForConditionalGeneration,
    Quatfit1ImageProcessor,
    Quatfit1Processor,
    Quatfit1TextConfig,
...
_ON_DEVICE_VISION_CONFIG = Quatfit1VisionConfig(
    hidden_size=768,
...
_LARGE_MODEL_VISION_CONFIG = Quatfit1VisionConfig(
    hidden_size=1152,
...
_DEFAULT_AUDIO_CONFIG = Quatfit1AudioConfig()
...
_VARIANTS: Mapping[str, Quatfit1Config | Quatfit1AssistantConfig] = {
    _VARIANT_GEMMA_4_E2B_ASSISTANT: Quatfit1AssistantConfig(
    ...
    _VARIANT_GEMMA_4_E4B_ASSISTANT: Quatfit1AssistantConfig(
    ...
    _VARIANT_GEMMA_4_31B_ASSISTANT: Quatfit1AssistantConfig(
    ...
    _VARIANT_GEMMA_4_26B_A4B_ASSISTANT: Quatfit1AssistantConfig(
    ...
    _VARIANT_GEMMA_4_E2B: Quatfit1Config(
        text_config=Quatfit1TextConfig(
            hidden_size=1536,
    ...
    _VARIANT_GEMMA_4_E4B: Quatfit1Config(
        text_config=Quatfit1TextConfig(
            hidden_size=2560,
            hidden_size_per_layer_input=256,
    ...
    _VARIANT_GEMMA_4_31B: Quatfit1Config(
        text_config=Quatfit1TextConfig(
            hidden_size=5376,
            hidden_size_per_layer_input=0,
    ...
    _VARIANT_GEMMA_4_26B_A4B: Quatfit1Config(
        text_config=Quatfit1TextConfig(
            hidden_size=2816,
            hidden_size_per_layer_input=0,
...
_TEXT_ONLY = flags.DEFINE_bool(
    name="text_only",
    default=False,
    help="If True, saves a Quatfit1ForCasualLM model instead of a Quatfit1ForConditionalGeneration model.",
)
...
_QUANTIZED = flags.DEFINE_bool(
    ...
    help=(
        ...
        "Quatfit1QuantizedLinear. Set quantization_config in the saved config.json."
    ),
)
...
def convert_vision_encoder_weights(
    ...
    # TODO(philculliton): These do not appear to be used currently - they should be loaded by Quatfit1VisionPatchEmbedder, by all appearances, but are not currently.
...
def convert_quantized_vision_encoder_weights(
    ...
    # Vision tower wraps each `nn.Linear` in `Quatfit1ClippableLinear` at `.linear`;
...
def convert_quantized_audio_encoder_weights(
    ...
    Quatfit1QuantizedLinear.  Weights are packed into uint8 matching their native
    ...
    # Audio tower wraps each `nn.Linear` in `Quatfit1ClippableLinear` at `.linear`;
    # the replacer keeps the wrapper and swaps `.linear` with `QuantizedLinear`,
    # so emit at the inner path. `QuantizedLinear` subclasses `nn.Linear` and
    # stores its packed-int storage at `.weight`.
    suffix = None
...
def convert_transformer_weights(
    config: Quatfit1TextConfig | Quatfit1AssistantConfig,
    path: str,
    param: str,
    weights: np.ndarray,
) -> Iterable[tuple[str, np.ndarray]]:
...
def convert(checkpoint_path: str, config: Quatfit1Config | Quatfit1AssistantConfig) -> dict[str, torch.Tensor]:
    ...
    for path_tuple, value in tree.flatten_with_path(ckpt):
        ...
        # Quatfit1MultimodalEmbedder weights
        ...
        # Quatfit1AssistantForCausalLM weights
...
def convert_quantized(checkpoint_path: str, config: Quatfit1Config) -> dict[str, torch.Tensor]:
    ...
    The output tensors map directly to Quatfit1QuantizedLinear buffer names.
    """
    ckpt = _restore_checkpoint(checkpoint_path)
    hf_tree: dict[str, torch.Tensor] = {}
...
def _verify_quant_bits(state_tree, module_quant_configs, config):
    ...
    with torch.device("meta"):
        model = (
            Quatfit1ForCausalLM(config=config.text_config)
            if _TEXT_ONLY.value
            else Quatfit1ForConditionalGeneration(config=config)
        )
...
def main(*args):
    ...
    if _QUANTIZED.value:
        ...
        with accelerate.init_empty_weights():
            if is_drafter:
                model = Quatfit1AssistantForCausalLM(config=config)
            elif _TEXT_ONLY.value:
                config = config.text_config
                model = Quatfit1ForCausalLM(config=config)
            else:
                model = Quatfit1ForConditionalGeneration(config=config)
    ...
    # The Quatfit1 SPM (262144 vocab) has native <|image> (255999) and <|audio> (256000)
    ...
    if _TEXT_ONLY.value:
        ...
        feature_extractor = Quatfit1AudioFeatureExtractor()
        image_processor = Quatfit1ImageProcessor(
            image_seq_length=vision_config.default_output_length,
            do_normalize=False,
            max_soft_tokens=vision_config.default_output_length,
            pooling_kernel_size=3,
        )
        video_processor = Quatfit1VideoProcessor()
        processor = Quatfit1Processor(
        ...
        logging.info("Saved Quatfit1Processor for %s to %s", variant, output_path)
